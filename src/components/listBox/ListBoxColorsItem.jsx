  ...
  return (
      <div>
        <Listbox  value={selectedColor} onChange={setSelectedColor}>
          {({ open }) => (
              <>
                <Listbox.Label className="text-sm font-medium leading-6 text-gray-900">Choisir une couleur</Listbox.Label>
              ...
              <span className="flex items-center">
                ...
                />
                <span className="ml-3 block truncate">{selectedColor.label}</span>
              </span>
                  ...
                  >
                    <Listbox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                      {colors.length > 0 ? colors.map((color, index) => (
                         ...
                         <InfoEmpty message={"aucune couleur trouvée"}/>
                      }
                    </Listbox.Options>
                  </Transition>
                </div>
